...
<div class="flex gap-1 w-full h-full bg-neutral-800 p-1">
    <div class="flex flex-col w-1/5 bg-neutral-900 rounded-l-md rounded-r-sm">
        {#each items as item (item)}
            <ContentBrowserListFolder item={item} select={(ids) => selected = ids} bind:selected={selected} />
        {/each}
    </div>
    <div class="flex flex-col h-full w-full">
        <div class="h-fit flex px-1 pt-1 pb-2 items-center">
            {#each selected as id, index (id)}
                <button class="text-xs py-1 px-2 cursor-pointer hover:bg-neutral-700 rounded-lg" onclick={() => selected = selected.slice(0, selected.indexOf(id) + 1)}>{findRecursive(items, id)?.name || 'unknown'}</button>
        ...
        <div class="bg-neutral-900 w-full h-full rounded-r-md rounded-l-sm flex gap-2 p-2">
            {#if selectedContent && selectedContent.children}
                {#each selectedContent.children as item (item)}
                    <ContentBrowserItemCard item={item} select={item.type === 'folder' ? () => selected.push(item.id) : () => {}}/>
                {/each}
            {/if}
        </div>
    </div>
